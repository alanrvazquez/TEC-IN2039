...
R tiene varias funciones para checar el tipo de variable que se está usando. Por ejemplo, consideremos el siguiente conjunto de datos:

```{r}
mediciones <- c(10,5, 10.2, 25, 23.036)
```

Es evidente que la variable `medicion` es una variable numérica porque contiene números que tienen decimales. Para asegurarnos que R sabe que esa variable es numérica usamos la función `is.numeric()`.

```{r}
is.numeric(mediciones)
```

La función arroja un valor booleano según sea el caso. Arroja `TRUE` si el vector es numérico y `FALSE` si no lo es.
