...
::: {style="font-size: 70%;"}
Estas diapositivas estan basadas en material disponible en <https://coursekata.org/>
:::
...
-   Se puede extender con paquetes (librerias) desarroladas por otros usuarios.
:::
...
::: notes
Para hacer graficos o visualizaciones se tiene que usar la computadora y programas especiales. Hay muchos como Python tableu, o incluso, excel. Sin embargo los mas bonitos son los de R.

El conocer r te ayudara a tu CV porque, despues de este cusro, puedes poner conocimiento basico de analysis de datos con R.
:::

## Empezemos con su instalación
...
Después de ejecutar este código, no susede nada. Pero si lo ejecutamos el objeto por si solo, podrémos ver lo qua hay en el.
...
1.  Escribe código para devidir el `mi_vector` por 2.
2.  Guarda el resultado en un nuevo vector llamado `mi_nuevo_vector`.
3.  Prueba el siguiente comando `sum(mi_vector_2)`. ¿Qué obtuviste?
...
Puedes pensar en los objetos R como depósitos que contienen valores. Un objeto R puede contener un solo valor o puede contener un grupo de valores (como en el caso de un vector).
...
# Aquí hay dos formas de crear un vector númerico con los numeros del 1 al 10.
...
Los caracteres se componen de texto, como palabras u oraciones. (Los números también se pueden tratar como caracteres, dependiendo del contexto.

Por ejemplo, cuando 20 está entre comillas como esta (`"20"`) se tratará como un valor de carácter, aunque incluya un número). entre comillas, `" "`.
...
Quizás tengamos una pregunta como: ¿El primer elemento del vector es `muchos_holas` `"hola"`?
...
Por ejemplo, `5 != 3`es una `TRUE` declaración.

## Mini-actividad (*solo* mode)

1.  Lee este código y predice su respuesta. Después, corre el código en RStudio y valida si estabas en lo correcto.
...
2.  Repite lo mismo con el siguiente codigo.
...
## Lista de operadores lógicos comúnes
...
-   Trabajaremos mayormente con las librerias *readxl* y *ggplot2*.

# ¿Cómo cargar datos en R?

## Organización de datos

En estadística, generalmente organizamos los datos en filas y columnas.
...
Las filas representan los casos muestreados. En este ejemplo, las filas representan amas de llaves de diferentes hoteles. Hay seis filas, por lo que hay seis amas de casa en este conjunto de datos.
...
Entonces una de las variables es `Condition` si fueron informados de este hecho o no.
...
En este curso, asumiremos que los datos estan almacenados en un archivo Excel con la organización anterior. Como ejemplo, usemos el archivo `prenguins.xlsx`.
...
[***El archivo debe de estar en la misma carpeta que tu código.***]{style="color:red;"}

## Libreria `readxl`

::: columns
::: {.column width="70%"}
Para cargar datos en R, usaremos la libreria llamada `readxl`. Primero, debes de instalar esta libreria en R utilizando el comando `install.packages("readxl")`
:::
...
<br/> Una vez instalada la libreria, la cargamos en R usando el siguiente comando.
...
::: columns
::: {.column width="80%"}
1.  Instala la libreria usando el comando `install.packages("ggplot2")`
:::
...
2.  Instala una libreria auxiliar `install.packages("ggformula")`

3\. Carga ambas libreries en R usando el siguiente código.
