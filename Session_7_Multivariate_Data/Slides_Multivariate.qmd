...
Las funciones se usan con una sintaxis especial que usa el operador `%>%` para indicas el orden entre las operaciones. 
...
Puedes guardar la tabla en un nuevo objeto y usar las mismas funciones que hemos visto para visualizar los resúmenes estadísticos en lugar de los datos originales.
...
gf_point(Promedio ~ origin, data = sum_data, size = 5)
```
...
## Gráfica de barras lado al lado

```{r}
#| fig-pos: center
#| echo: true

auto_data$year <- factor(auto_data$year)

gf_bar( ~ origin, data = auto_data, fill = ~year, position = position_dodge())
```
