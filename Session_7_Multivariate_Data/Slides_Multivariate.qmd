...
Carguémos las librerías `readxl`, `ggplot2`, y `ggformula` en Google Colab y R antes de comenzar.
...
my_scatter = gf_point(mpg ~ weight, color = ~origin, data = auto_data) 
my_scatter
```
...
my_facet = gf_point(mpg ~ weight, data = auto_data) %>% gf_facet_grid( ~ origin)
my_facet
```
