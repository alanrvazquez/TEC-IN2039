...
De ser necesario, también podemos ajustar el eje horizontal para mostrar el 0.
...
La gráfica más efectiva para estudiar la relación entre una variable categórica y una numérica es la [gráfica de cajas lado al lado]{style="color:darkblue;"}.
...
También, podemos gráficar una curva de densidad de la variable $Y$ para cada subgrupo definido por $Z$. Para esto, usamos el argumento `fill` igual a la variable $Z$. 
...
gf_point(Promedio ~ origin, data = sum_data, shape = 4, size = 5)
```
...
Asegurate que sean especificados como `factor` en R.
...
Por ejemplo, para estudiar la distribución de las especies de pinguinos en las tres diferentes islas usamos lo siguiente:
...
La gráfica muestra el la frecuencia de cada especie, separada por el nombre de la isla.  
...
Ventajas de los gráficos de barras lado a lado:
...
Para variables categóricas, los resúmenes estadísticos más comunes son la frequencia y la frecuencia relativa.

El siguiente código muestra como calcular la frecuencia (`n`) y la frecuencia relativa (`Proporción`) usando funciones de la libreria `dplyr`.
...
Crea una gráfica para cada subgrupo de $Z$. 

La gráfica se crea usando la función `gf_facet_grid` con la sintaxis `%>%` de `dplyr`.
...
La función produce una rejilla con una columna y tres filas de gráficas. 
...
Una gráfica común para cuatro variables es la gráfica de dispersión donde el color y el tamaño de los símbolos dependen de dos variables adicionales **categóricas**.
