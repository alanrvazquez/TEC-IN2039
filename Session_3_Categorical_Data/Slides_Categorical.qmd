...
Hoy usaremos R para construir gráficas y tablas. Para esto, usaremos las librerías que ya vimos antes como `readxl`, `ggplot2`, `ggformula`, y `plotly`.
...
Y si queremos hacer la gráfica más interactiva, podemos usar `plotly`.
...
my_plot = gf_percents( ~ species, data = penguins_data, fill = ~species) + labs(title = "Proporciones de pingüinos", subtitle = "Distribución pingüinos en Adelie, Chinstrap, and Gentoo", x = "Tipo de especie", y = "Frecuencia relativa", fill = "Especie") 
ggplotly(my_plot)
```
