...
[**Objetivo:**]{.underline} *Enfocarnos en 1 variable numérica.*
...
::: {.column width="50%"}
2.  Histogramas y gráficas de caja
:::
:::
...
## Primero, carguemos los datos en R

```{r}

library(readxl) # Para leer datos.
library(ggplot2) # Para visualizar.
library(ggformula) # Para usar gáficas.
...
-   Calcula el primedio, varianza, desviación estándard, cuartiles, mínimo y máximo.
...
gf_histogram( ~ cycle_time, data = piston_data, bins = 13, color = "black",
...
## ¿Cómo fijo el número de intervalos?

[El número de intervalos afecta el aspecto del histograma. El histograma izquierdo usa `bins = 5` y el histograma derecho usa `bins = 30`.]{style="font-size: 100%;"}
...
-   La función `gf_histogram` fija el número de intervalos usando el parámetri `bins` que se fija en 30 por default.
